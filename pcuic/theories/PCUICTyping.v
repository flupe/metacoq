...
Section GlobalMaps.
      ...
      cshape_eq : ctype = it_mkProd_or_LetIn mdecl.(ind_params)
                            (it_mkProd_or_LetIn cshape_args
                              (mkApps cshape_concl_head
                                      (to_extended_list_k mdecl.(ind_params) #|cshape_args| ++ cshape_indices)))
  ...
  Definition on_constructor Σ mdecl i idecl cdecl cuniv :=
  ...
  Definition check_constructors_smaller (Σ : global_env_ext) ctrsu indu :=
    Forall (fun s => leq_universe Σ s indu) ctrsu.
  ...
  Definition check_ind_sorts Σ kelim ctrsu indu :=
    match universe_family indu with
    | InProp =>
  ...
  Record on_ind_body Σ mind mdecl i idecl :=
    { (** The type of the inductive must be an arity, sharing the same params
      ...
      onConstructors : on_constructors Σ mdecl i idecl idecl.(ind_ctors) ind_ctors_sort;
      (** Projections, if any, are well-typed *)
      onProjections : idecl.(ind_projs) <> [] -> on_projections Σ mind mdecl i idecl ind_indices;
      ...
      ind_sorts : check_ind_sorts Σ idecl.(ind_kelim) ind_ctors_sort ind_sort;
    }.
